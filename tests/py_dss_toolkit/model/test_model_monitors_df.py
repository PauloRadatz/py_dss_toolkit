...
New Line.L1 bus1=A bus2=B phases=3 r1=0.1 x1=0.1 c1=0 length=1
New Load.L1 bus1=B kw=50 pf=1
New Monitor.Mon1 element=Line.L1 terminal=1 mode=1 enabled=no
Set voltagebases=[13.8]
Calcvoltagebases
Solve
"""
...
def test_monitors_df_enabled_returns_row():
    run_dss_script(SCRIPT_ENABLED)
    df = dss_tools.model.monitors_df
    assert df is not None
    assert set(df["name"].str.lower()) == {"mon1"}


def test_monitors_df_disabled_returns_none():
    run_dss_script(SCRIPT_DISABLED)
    df = dss_tools.model.monitors_df
    assert df is None


def test_monitors_df_none_when_no_monitors():
