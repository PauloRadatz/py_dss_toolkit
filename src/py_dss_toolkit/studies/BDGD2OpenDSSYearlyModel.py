...
def _get_br_holidays(year: int) -> set:
...
class BDGD2OpenDSSYearlyModel(StudyBase):
    """
    ...
    def __post_init__(self):
        ...
        if self._dss_file is None:
            self._dss_file = self._find_master_file()
    ...
    def _initialize_views(self):
        """Set up study view layers.

        BDGD yearly model generation is a file-conversion workflow, so it does
        not expose result/view helpers like power-flow studies do.
        """

    def _find_master_file(self):
    ...
    def _parse_contagem_dias(self):
        ...
        csv_files = [
            f for f in os.listdir(csv_folder) if f.lower().startswith("contagem_dias") and f.lower().endswith(".csv")
        ]
        if not csv_files:
        ...
        with open(csv_path, "r", encoding="utf-8-sig") as f:
            ...
            for row in reader:
                ...
                ano = int(row_cleaned["Ano"])
                du = int(float(row_cleaned["DU"]))
                sa = int(float(row_cleaned["SA"]))
                do = int(float(row_cleaned["DO"]))

                day_counts[month] = {"DU": du, "SA": sa, "DO": do, "total": du + sa + do, "year": ano}
        return day_counts
    ...
    def _build_calendar(self, day_counts):
        calendar_dict = {}
        year = day_counts[1]["year"]
        br_holidays = _get_br_holidays(year)
        ...
        for month in range(1, 13):
            ...
            d = date(year, month, 1)
            while d.month == month:
                if d in br_holidays or d.weekday() == 6:
            ...
            counts = {
                "DU": standard_month.count("DU"),
                "SA": standard_month.count("SA"),
                "DO": standard_month.count("DO"),
            }
    ...
    def _parse_curvacarga(self):
        ...
        with open(curva_files[0], "r", encoding="utf-8") as f:
            for line in f:
                if line.strip().lower().startswith('new "loadshape.') or line.strip().lower().startswith(
                    "new loadshape."
                ):
    ...
    def _process_loads(self, prefix, calendar_dict, shapes_24h):
        ...
        for name, data in loads.items():
            ...
            final_loads[name] = {"max_kw": max_kw, "curve": norm_curve, "template": data["def"]}
    ...
    def _write_cargas(self, path, loads):
        with open(path, "w", encoding="utf-8") as f:
            for name, data in loads.items():
                line = data["template"]
                # Replace kw
                line = re.sub(r"(kw\s*=\s*)([\d\.]+)", rf"\g<1>{data['max_kw']:.6f}", line, flags=re.IGNORECASE)
                # Replace daily with yearly
                line = re.sub(r'(daily\s*=\s*)"?([^"\s]+)"?', rf'yearly="{name}_8760"', line, flags=re.IGNORECASE)
                f.write(line + "\n")
    ...
    def _write_master_and_copy_grid(
        self,
        output_folder: str,
        total_hours: int,
        add_nt_monthly_redirects: bool = False,
        ...
        with open(base_master, "r", encoding="utf-8") as fin, open(new_master_path, "w", encoding="utf-8") as fout:
            ...
            for line in fin:
                ...
                if re.match(r"^set\s+tolerance\s*=", line_strip, re.IGNORECASE):
                    continue
                if re.match(r"^set\s+maxiterations\s*=", line_strip, re.IGNORECASE) or re.match(
                    r"^set\s+maxi\s*=", line_strip, re.IGNORECASE
                ):
                    continue
                ...
                if re.match(r"^!?\s*(?:redirect|compile)\s+", line_strip, re.IGNORECASE):
                    ...
                    if file_match:
                        ...
                        if (
                            lower_fname.startswith("cargasbt")
                            or lower_fname.startswith("cargasmt")
                            or lower_fname.startswith("curvacarga")
                        ):
                            ...
                            if (
                                lower_fname.startswith("gd_bt") or lower_fname.startswith("gd_mt")
    ...
    def _create_loading_info(
        self, output_folder, calendar_dict, tolerance_pf: float = 0.0001, max_iterations_pf: int = 15
    ):
        ...
        summary = [
            {
                "Scope": "Global",
                "Start hour": 1,
                "End hour": len(total_kw),
                "Peak Value (kW)": round(global_peak_val, 2),
                "Peak Timestep": global_peak_ts,
                "Off-Peak Value (kW)": round(global_offpeak_val, 2),
                "Off-Peak Timestep": global_offpeak_ts,
            }
        ]
    ...
    def execute_base_case(
        ...
        self._write_master_and_copy_grid(
            output_folder, total_hours, tolerance_pf=tolerance_pf, max_iterations_pf=max_iterations_pf
        )
        ...
        if create_loading_info_file:
            ...
            self._create_loading_info(
                output_folder, calendar_dict, tolerance_pf=tolerance_pf, max_iterations_pf=max_iterations_pf
            )

        return output_folder

    def execute_annual_energy_case(self, *args, **kwargs):
        return self.execute_monthly_energy_case(*args, **kwargs)
    ...
    def execute_monthly_energy_case(
        ...
        self._write_master_and_copy_grid(
            output_folder,
            total_hours,
            add_nt_monthly_redirects=True,
            tolerance_pf=tolerance_pf,
            max_iterations_pf=max_iterations_pf,
        )

        # 2. Extract target energy from CircMT_ per month
        csv_folder = os.path.join(self.dss_model_folder, "csv_files")
        circ_mt_files = [
            f for f in os.listdir(csv_folder) if f.lower().startswith("circmt") and f.lower().endswith(".csv")
        ]
        ...
        def process_nt_loads(loads_dict, load_type, nt_list):
            for k, v in loads_dict.items():
                ...
                line = v["template"]
                line = re.sub(rf"(?i)(Load\.{re.escape(k)})", rf"\1_NT", line)
                line = re.sub(r'(kw\s*=\s*)"?([^"\s]+)"?', "", line, flags=re.IGNORECASE)
                line = re.sub(
                    r'((?:daily|yearly)\s*=\s*)"?([^"\s]+)"?', rf'yearly="{k}_NT_8760"', line, flags=re.IGNORECASE
                )
        ...
        for m in range(1, 13):
            ...
            for iteration in range(1, max_iterations + 1):
                ...
                if not dss.solution.converged:
                    raise RuntimeError(
                        f"Power flow did not converge during Month {m}, Iteration {iteration}. "
                        f"Current max_iterations is {dss.solution.max_iterations} and tolerance is {dss.solution.tolerance}. "
                        f"You can try to change the maxiteration for the power flow (suggested: between 30 and 50) "
                        f"before changing the tolerance, or increase the tolerance by 10 times."
                    )
